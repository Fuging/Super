...
spawn(function()
    wait(5)  -- Tunggu 5 detik untuk memastikan game fully loaded
    ensureThermometerClone()
    
    -- Coba aktifkan ulang thermometer clone setiap 10 detik untuk memastikan tetap menyala
    while true do
        task.wait(10)
        if clonedThermometer then
            -- Cek apakah thermometer masih menyala dengan membaca display
            local temp = readThermometerDisplay(clonedThermometer)
            if temp == "Unknown" then
                print("Cloned thermometer appears to be off, reactivating...")
                createThermometerClone()
            end
        end
    end
